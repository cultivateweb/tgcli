...
TUI открывается мгновенно из локального кеша (`~/.config/tgcli/cache.db`), а сеть
обновляет данные в фоне; входящие сообщения прилетают в реальном времени.

## Дальнейшие шаги

См. `docs/USER_STORIES.md` — демон (фоновый онлайн), desktop-уведомления,
вкладки по типам чатов и прокрутка истории вглубь.
